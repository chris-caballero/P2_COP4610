...
int thread_run(void *data) {
    ...
    while(!kthread_should_stop()) {
        if(mutex_lock_interruptible(&param->mutex) == 0) {
            ...
            // } else {
            //     //if floor is 0
            //         //for each in elevator find closest next floor
            //         //state UP
            //     //if floor > 0
            //         //if elevator.size == 0
            //             //if waiting threads
            ...
            switch(param->state) {
                case OFFLINE:
                    //for now, eventually just get woke by thread?
                    for(i = 0; i < NUM_FLOORS; i++) {
                        if(building.floors[i]->size > 0) {
                            next_stop = building.floors[i]->height;
                            break;
                        }
                    }
                    if(next_stop > 0) {
                    ...
                    break;
                case IDLE:
                    //wait on a passenger to wake it
                    param->direction = 0;
                    break;
                case LOADING:
                    if(param->direction == 1) {
                        unload_elevator(building.floors[param->current_floor]);
                    } else if(param->direction < 1) {
                        load_elevator(building.floors[param->current_floor]);
                    }
                    
                    if((next_stop = get_next_stop()) == -1) {
                        param->state = OFFLINE;
                    }
                    param->state = next_stop > param->current_floor ? UP : DOWN;
                    ssleep(2);
                    break;
                case UP:
                    ssleep(1);
                ...
                case DOWN:
                    ssleep(1);
                    param->direction = -1;
                    if(building.floors[param->current_floor]->size > 0) {
                        param->state = LOADING;
                    }
                    if(param->current_floor == next_stop) {
                        param->state = LOADING;
...
int get_next_stop(void) {
    ...
    list_for_each(temp,  &elevator.list) {
        ...
        next_stop = min(p->destination_floor, next_stop);
    }
    return next_stop;
}
...
int add_passenger(int start_floor, int destination_floor, int type) {
    ...
    for(i = 0; i < NUM_FLOORS; i++) {
        curr_floor = building.floors[i];
        if(start_floor == curr_floor->height) {
            break;
        }
    }

    list_add_tail(&new_passenger->list, &curr_floor->list);
    curr_floor->size += 1;
